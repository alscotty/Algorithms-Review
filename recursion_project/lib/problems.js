...
function sumArray(array) {
    ...
    return sumArray(array.slice(1)) + array.shift()
}
...
function pow(base, exponent) {
    ...
    } else { return 1 /(base * pow(base, Math.abs(exponent) - 1))}
    ;
}
...
// return (data.map(el=> typeof el==='array' ? flatten(el) : [el]))

function flatten(data) {
    if(data instanceof Array ===false){return [data]}
...
function fileFinder(directories, targetFile) {
    ...
    //     if(subfile[0]=='/'){
    //         fileFinder(subfile,targetFile)
    //     } else{
    //         subfile.forEach(regFile=>{
    //             if(regFile==targetFile){
    //                 return true;
    //             }
    //         })
    //     }
