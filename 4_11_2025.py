...
def return_mismatched_words(str1, str2):
  ...
  for str1_word in str1_words:
    if str1_word not in str2_words:
      uniq_words.append(str1_word)
      
  for str2_word in str2_words:
    if str2_word not in str1_words:
      uniq_words.append(str2_word)
...
def return_smallest_key(inputDict, n):
  ...
  for name,value in inputDict.items():
    el_list.append({ "name": name, "value": value })
  
  el_list.sort(key=get_val)
  desired_idx = n-1
...
def return_missing_balanced_numbers(input):
  ...
  required_dict_to_balance = {}
  max_count = max(seen_dict.values())
  for key, count in seen_dict.items():
    if count < max_count:
      required_dict_to_balance[key] = max_count - count
  
  return required_dict_to_balance
...
def fill_in_the_blanks(input_lst):
  ...
  for idx in range(len(input_lst)):
    if input_lst[idx] == None:
      input_lst[idx] = most_recent_non_null
    else:
      most_recent_non_null = input_lst[idx]
