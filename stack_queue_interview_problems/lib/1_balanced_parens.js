...
function balancedParens(str) {
    ...
    let leftBrackets=list.slice(0,midIdx);
    let rightBrackets=list.slice(midIdx);
    if(leftcount===0 || rightcount===0) return false;
    if (leftBrackets.every(el => el === '(') && rightBrackets.every(el => el === ')') && (leftcount===rightcount) ) return true;
    return false
}
